...
public class TwitchClient {
    private static final String TOKEN = "Bearer hd4baxvw645xgnz284vwz9cnyd3hd4";
    private static final String CLIENT_ID = "bemiodgy0mlrrzrio67x29necp0vpo";
    ...
    private static final String TWITCH_BASE_URL = "https://www.twitch.tv/"; //eg. /brocastname to find other game
    ...
    /*
     * check which url
     * deal with top url or search url
     */

    /*--------------------------Search Game by GameName via searchGame url begining -------------------*/
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) { // gamename is empty, then it is top game url with first 20 games
            return String.format(url, limit); //
        } else { // game is not empty, then it is game url
            try {
                // Encode special characters in URL, e.g. name = League of Legend -> League%20ofLegned
                gameName = URLEncoder.encode(gameName, "UTF-8"); // speical char change to UTF-8 format
    ...
    private String searchTwitch(String url) throws TwitchException {
        ...
        ResponseHandler<String> responseHandler = response -> {
            ...
            JSONObject obj = new JSONObject(EntityUtils.toString(entity)); // two object: data, pagenation
            return obj.getJSONArray("data").toString();// get the objectArray of key is data;
        };
    ...
    /*--------------------------Search Game by GameName via searchGame url end -------------------*/
